...
void MainApplication::InitScene() {
    ...
    const Vector3 EYE_POSITION(0.0f, 10.0f, -1.0f);
    ...
    mainCamera->SetViewport(0, 0, gCGKitInterface.GetScreenWidth(),
                            gCGKitInterface.GetScreenHeight());
    ...
    if (model != nullptr) {
        ...
        if (mesh != nullptr) {
            ...
            for (u32 i = 0; i < subMeshCnt; ++i) {
                ...
                Material *material =
                        dynamic_cast<Material *>(gResourceManager.Get(
                                ResourceType::RESOURCE_TYPE_MATERIAL));
                if (material == nullptr) {
                ...
                material->SetSamplerParam(TextureType::TEXTURE_TYPE_ENVIRONMENTMAP,
                                          ...
                                          SAMPLER_MIPMAP_BILINEAR,
                                          SAMPLER_ADDRESS_CLAMP);
                material->AttachShaderStage(ShaderStageType::SHADER_STAGE_TYPE_VERTEX,
                                            "shaders/pbr_vert.spv");
                material->AttachShaderStage(ShaderStageType::SHADER_STAGE_TYPE_FRAGMENT,
...
void MainApplication::Update(float deltaTime) {
    ...
    const float POINT_HZ_X = 0.4f;
    const float POINT_HZ_Y = 1.0f;
    ...
    if (m_pointLightObject) {
        m_pointLightObject->SetPosition(
                Vector3(sin(m_deltaAccumulate * POINT_HZ_X) * POINT_LIGHT_CIRCLE,
                        sin(m_deltaAccumulate * POINT_HZ_Y) * POINT_LIGHT_CIRCLE +
                        POINT_LIGHT_CIRCLE,
...
SceneObject *MainApplication::CreateSkybox() {
    ...
    for (u32 i = 0; i < subMeshCnt; ++i) {
        SubMesh *subMesh = mesh->GetSubMesh(i);
        // add Material
        Material *material = dynamic_cast<Material *>(gResourceManager.Get(
                ResourceType::RESOURCE_TYPE_MATERIAL));
        material->Init();
        material->SetSubMesh(subMesh);
        material->SetTexture(TextureType::TEXTURE_TYPE_ENVIRONMENTMAP, m_envMap);
        material->SetSamplerParam(TextureType::TEXTURE_TYPE_ENVIRONMENTMAP,
                                  ...
                                  SAMPLER_ADDRESS_CLAMP);
        material->AttachShaderStage(ShaderStageType::SHADER_STAGE_TYPE_VERTEX,
                                    "shaders/sky_vert.spv");
        material->AttachShaderStage(ShaderStageType::SHADER_STAGE_TYPE_FRAGMENT,
                                    "shaders/sky_frag.spv");
        material->SetCullMode(CULL_MODE_NONE);
        material->SetDepthTestEnable(true);
        material->SetDepthWriteEnable(true);
        material->Create();
