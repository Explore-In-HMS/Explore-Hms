 ...
 *
 */
package com.genar.hmssandbox.huawei.feature_mlkit.ui.cameraOperations.graphicView;
...
public class FaceGraphic extends BaseGraphic {
    ...
    /**
     * MLFaceGraphic constructor
     * makes set GraphicOverlay with overlay params and set
     * makes set and draws GraphicOverlay such as drawing face drawing facial contours and face analyser contents
     * makes graphic overlay such as eyePointsPaint, noisePointsPaint, lipPointsPaint
     *
     * @param overlay        : GraphicOverlay
     * @param faces          : List<MLFace>
     * @param context        : List<MLFace>
     * @param isOpenFeatures : boolean
     */
    public FaceGraphic(GraphicOverlay overlay, List<MLFace> faces, Context context, boolean isOpenFeatures) {
        super(overlay);
        ...
        this.textPaint.setTextSize(dp2px(this.mContext, 6));
        this.textPaint.setTypeface(Typeface.DEFAULT);

        this.faceFeaturePaintText = new Paint();
        this.faceFeaturePaintText.setColor(Color.WHITE);
        this.faceFeaturePaintText.setTextSize(dp2px(this.mContext, 11));
        ...
        this.faceFeaturePaint.setStrokeWidth(dp2px(this.mContext, 2));
        ...
        this.landmarkPaint.setStrokeWidth(dp2px(this.mContext, 2));
    ...
    public List<String> sortHashMap(Map<String, Float> map) {
        Set<Map.Entry<String, Float>> entey = map.entrySet();
        List<Map.Entry<String, Float>> list = new ArrayList<>(entey);
     ...
     * such as drawing face drawing facial contours and face analyser contents
     * such as
     * "Smiling", mFace.getEmotions().getSmilingProbability()
     * "Angry", mFace.getEmotions().getAngryProbability()
     *
     * @param canvas
     */
    @Override
    public void draw(Canvas canvas) {
        ...
        if (this.isLandScape()) {
            ...
            y = (dp2px(this.mContext, this.overlay.getHeight() / 16.0f)) < 340.0 ? 340 : (dp2px(this.mContext, this.overlay.getHeight() / 16.0f));
        ...
        if (this.isOpenFeatures) {
            if (this.faces.size() > 0) {
                List<MLFace> faceFeatures = new ArrayList<>();
            ...
            for (MLFaceKeyPoint landmark : face.getFaceKeyPoints()) {
                if (landmark != null) {
                    ...
                    canvas.drawCircle(
                            ...
                            dp2px(this.mContext, 3), this.landmarkPaint);
                }
            }
        }
    }
