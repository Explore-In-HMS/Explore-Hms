...
import com.hms.explorehms.huawei.feature_arengine.ui.world.VirtualObject;
import com.hms.explorehms.huawei.feature_arengine.utils.MatrixUtil;
...
public class ObjectDisplay {
    ...
    private final float[] mViewLightDirections = new float[4];
    ...
    private final float[] mModelViewMatrixs = new float[MATRIX_SIZE];

    private final float[] mModelViewProjectionMatrixs = new float[MATRIX_SIZE];

    // The largest bounding box of a virtual object, represented by two diagonals of a cube.
    private final float[] mBoundingBoxs = new float[6];
    ...
    public boolean hitTest(float[] cameraView, float[] cameraPerspective, VirtualObject obj, MotionEvent event) {
        ...
        return ((event.getX() > boundarys[0]) && (event.getX() < boundarys[1]))
                && ((event.getY() > boundarys[2]) && (event.getY() < boundarys[3]));
    }
